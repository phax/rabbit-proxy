...
public class HtmlParser
{
   ...
   */
  public HtmlParser (final Charset cs)
  {
    this.cs = cs;
    pagepart = null;
   ...
   *        the block to parse.
   */
  public void setText (final byte [] page)
  {
   ...
   */
  public void setText (final byte [] page, final int startIndex, final int length)
  {
    init ();
    csDecoder.reset ();
    final ByteBuffer bb = ByteBuffer.wrap (page, startIndex, length);
  ...
  private int scanString ()
  {
    ...
    if (tagmode)
    {
      stringValue = new String (pagepart, startindex, (endindex - startindex));
    }
    else
    {
  ...
  private int scanQuotedString ()
  {
    int endindex = -1;
    final int startindex = index - 1;
    final char start = pagepart[startindex];
    while (index < length)
    {
    ...
    final int l = (endindex < length ? endindex : length) - startindex;
   ...
   *         if the match fails
   */
  private int match (final int token) throws HtmlParseException
  {
    int ts;
  ...
  private void setPendingComment (final Token comment)
  {
    nextToken = SCRIPT;
   ...
   */
  private void arglist (final Tag tag) throws HtmlParseException
  {
    String key;
      ...
      switch (nextToken)
      {
        case MT:
          ...
          if (tag.getLowerCaseType () != null &&
              (tag.getLowerCaseType ().equals ("script") || tag.getLowerCaseType ().equals ("style")))
          {
            final Token text = scanCommentUntilEnd (tag.getLowerCaseType ());
            if (text != null)
            {
        ...
        case END:
          return;
        case DQSTRING:
          final String ttype = tag.getType ();
          ...
          else
          {
             ...
             * //System.err.println ("hmmm, strange arglist..: " + tag); // this
             * is probably due to something like: // ... framespacing="0""> //
             * we backstep and change the second '"' to a blank and // restart
             * from that point... index -= stringValue.length ();
             * pagepart[index] = ' '; match (nextToken); tag.getToken
             * ().setChanged (true);
             */
          }
          break;
        case LT:
          ...
          final String type = tag.getLowerCaseType ();
   ...
   */
  private boolean sameTag (final String tag, final int j)
  {
    int i;
    for (i = 0; i < tag.length () && i < length; i++)
    {
      final char lb = Character.toLowerCase (tag.charAt (i));
      final char pb = Character.toLowerCase (pagepart[j + i]);
      if (lb != pb)
        return false;
    }
    return true;
  ...
  private Token scanCommentUntilEnd (final String tag)
  {
    final int len = tag.length ();
    final int startvalue = index;
    int i = -1;
    int j = index;
    ...
    if (i > -1)
    {
      ...
      index = j;
      return new Token (pagepart, TokenType.COMMENT, startvalue, stringLength);
    }
    block.setRest (lastTagStart);
    return null;
  }
   ...
   */
  private void tag (final int ltagStart) throws HtmlParseException
  {
    final Tag tag = new Tag ();
    final Token token = new Token (tag, false);
    switch (nextToken)
    {
      case STRING:
  ...
  private Token getToken (final TokenType type)
  {
