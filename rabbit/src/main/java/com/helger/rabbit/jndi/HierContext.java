...
{
  protected Hashtable <Object, Object> myEnv = new Hashtable <> ();
  ...
  HierContext (final Hashtable <?, ?> env)
  {
    if (env != null)
    {
      for (final Map.Entry <?, ?> me : env.entrySet ())
        myEnv.put (me.getKey (), me.getValue ());
    }
  }

  protected HierContext (final HierContext parent,
                         final String name,
                         final Hashtable <?, ?> inEnv,
  ...
  protected Context createCtx (final HierContext parent, final String name, final Hashtable <?, ?> inEnv)
  {
    return new HierContext (parent, name, inEnv, new HashMap <String, Object> ());
  }
  ...
  public Object lookup (final Name name) throws NamingException
  {
    ...
    if (name.size () == 1)
    {
      ...
      try
      {
        final Name cn = new CompositeName ().add (atom);
    ...
    {
      final String err = atom + " does not name a context, " + "bindings: " + bindings;
  ...
  public void bind (final Name name, Object obj) throws NamingException
  {
    if (name.isEmpty ())
    {
    ...
    else
    {
      ...
      if (inter == null)
      {
        final Context c = createSubcontext (atom);
        ...
        {
          final String err = "Intermediate already bound and " +
                             "it is not a context: name: " +
                             name +
                             ", inter: " +
                             inter;
          throw new NotContextException (err);
  ...
  public void rename (final Name oldname, final Name newname) throws NamingException
  {
    ...
    {
      final String err = "Do not support rename across different contexts";
      throw new OperationNotSupportedException (err);
    }

    final String oldatom = oldname.get (0);
    final String newatom = newname.get (0);
    ...
    else
    {
      ...
      ((Context) inter).rename (oldname.getSuffix (1), newname.getSuffix (1));
    }
  }
  ...
  public Context createSubcontext (final Name name) throws NamingException
  {
    ...
    if (name.size () == 1)
    {
      ...
      // Create child
      final Context child = createCtx (this, atom, myEnv);
  ...
  public Name composeName (final Name name, final Name prefix) throws NamingException
  {
    ...
    final String err = "Do not support composing composite names";
    throw new OperationNotSupportedException (err);
  }
  ...
  public String getNameInNamespace () throws NamingException
  {
    ...
    final Name name = myParser.parse ("");
  ...
  private abstract class ListOf <T extends NameClassPair> implements NamingEnumeration <T>
  {
    protected Iterator <String> names;

    ListOf (final Collection <String> names)
    {
  ...
  {
    ListOfNames (final Collection <String> names)
    {
      super (names);
    }
    ...
    public NameClassPair next () throws NamingException
    {
      ...
      final String className = bindings.get (name).getClass ().getName ();
  ...
  private class ListOfBindings extends ListOf <Binding>
  {
    ...
    ListOfBindings (final Collection <String> names)
    {
      super (names);
    }
