...
class UnCompressor implements GZipUnpackState
{
   ...
   *        the data listener
   * @param gzip
   *        if true use gzip handling with crc and trailers, if false use simple
   *        deflate handling.
   */
  public UnCompressor (final GZipUnpackListener listener, final boolean gzip)
  {
  ...
  {
    final byte [] unpacked = listener.getBuffer ();
    int num;
    try
    {
    ...
    if (num == 0)
    {
      ...
      if (inf.finished () || inf.needsDictionary ())
      {
        ...
        if (gzip)
        {
          final long length = inf.getBytesWritten ();
          final long cs = crc.getValue ();
