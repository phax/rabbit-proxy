...
class MagicReader implements GZipUnpackState
{
  ...
  public MagicReader (final GZipUnpackListener listener)
  {
  ...
  public void handleBuffer (final GZipUnpacker unpacker, final byte [] buf, int off, int len)
  {
    ...
    while (len > 0 && pos < GZIP_ID.length)
    {
      ...
      {
        final Exception e = new IllegalArgumentException ("gzip header not found: " +
                                                          pos +
                                                          ", " +
                                                          buf[off] +
                                                          " != " +
    ...
    {
      final GZipUnpackState cmr = new CompressionMethodReader (listener);
